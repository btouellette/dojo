/* MetaInfo - Holds all information gotten from a torrent file. Copyright (C) 2003 Mark J. Wielaard
 * 
...
public class MetaInfo
{
	...
	MetaInfo(String announce, String name, List<List<String>> files, List<Long> lengths, int piece_length, byte[] piece_hashes, long length)
	{
	...
	/**
	 * Return the length of a piece. All pieces are of equal length except for the last one (<code>getPieces()-1</code>).
	 * 
	...
	{
		return "MetaInfo[info_hash='" + hexencode(info_hash) + "', announce='" + announce + "', name='" + name + "', files=" + files + ", #pieces='" + piece_hashes.length / 20 + "', piece_length='" + piece_length + "', length='" + length + "']";
	}
	...
	{
		return new MetaInfo(announce, name, files, lengths, piece_length, piece_hashes, length);
	}
