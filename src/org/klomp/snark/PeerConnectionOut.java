/* PeerConnectionOut - Keeps a queue of outgoing messages and delivers them. Copyright (C) 2003 Mark J. Wielaard
 * 
...
class PeerConnectionOut implements Runnable
{
	...
	/**
	 * Continuesly monitors for more outgoing messages that have to be send. Stops if quit is true of an IOException occurs.
	 */
	public void run()
	{
		try {
	...
	void sendCancel(Request req)
	{
		...
		synchronized (sendQueue) {
			...
			while (it.hasNext()) {
				Message m = (Message) it.next();
				if (m.type == Message.REQUEST && m.piece == req.piece && m.begin == req.off && m.length == req.len) {
					it.remove();
				}
			}
		}
