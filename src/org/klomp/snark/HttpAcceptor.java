/* HttpAcceptor - Accepts incomming http connections. Copyright (C) 2003 Mark J. Wielaard
 * 
...
public class HttpAcceptor
{
	...
	/**
	 * Creates a HttpAcceptor that can handle torrent metadata of the given Tracker.
	 */
	...
	public void connection(Socket sock, BufferedInputStream bis, BufferedOutputStream bos) throws IOException
	{
	...
	/**
	 * Processes an incoming HTTP request. Only handles the most basic GET requests. Returns the (URLEncoded) requested resource or null if the request wasn't a valid GET request.
	 */
	...
	/**
	 * Consumes all headers and puts them into a Map mapping header value to header key Strings.
	 */
	 ...
	 */
	private static void sendData(OutputStream out, byte[] data, String content_type) throws IOException
	{
		sendData(out, 200, "OK", data, content_type);
	}

	private static void sendData(OutputStream out, int responseCode, String reason, byte[] data, String content_type) throws IOException
	{
		log.log(Level.FINER, "HTTP/1.0 " + responseCode + " " + reason + " " + content_type + " (" + data.length + " bytes)");
	...
	private static void sendError(OutputStream out, int responseCode, String reason) throws IOException
	{
		sendData(out, responseCode, reason, reason.getBytes(ASCII), "text/plain");
	}

	/**
	 * Returns a key to value map of the GET request query string parameters. It expects a '?' and the urlencoded key=value pairs. Note that the key and value are NOT url decoded before putting in the paramaters map.
	 */
