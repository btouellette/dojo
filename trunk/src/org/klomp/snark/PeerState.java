/* PeerState - Keeps track of the Peer state through connection callbacks. Copyright (C) 2003 Mark J. Wielaard
 * 
...
class PeerState
{
	...
	{
		log.log(Level.FINEST, peer + " rcv " + (interest ? "" : "un") + "interested");
		interested = interest;
	...
	void bitfieldMessage(byte[] bitmap)
	{
		synchronized (this) {
			...
			if (bitfield != null) {
				...
				log.log(Level.FINER, "Got unexpected bitfield message from " + peer);
				return;
			}
	...
	void requestMessage(int piece, int begin, int length) throws IOException
	{
	 ...
	 */
	void pieceMessage(Request req) throws IOException
	{
		int size = req.len;
		downloaded += size;
	...
	{
		log.log(Level.FINEST, "Got cancel message (" + piece + ", " + begin + ", " + length + ")");
		out.cancelRequest(piece, begin, length);
	}

	void unknownMessage(int type, byte[] bs)
	{
		log.log(Level.WARNING, "Ignoring unknown message type: " + type + " length: " + bs.length);
	}
