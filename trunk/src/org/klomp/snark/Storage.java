/* Storage - Class used to store and retrieve pieces. Copyright (C) 2003 Mark J. Wielaard
 * 
...
public class Storage
{
	...
	private static void addFiles(List<File> l, File f)
	{
		if (!f.isDirectory()) {
			...
			if (files == null) {
				log.log(Level.WARNING, "Skipping '" + f + "' not a normal file.");
				return;
			}
	...
	private void checkCreateFiles() throws IOException
	{
		...
		for (int i = 0; i < rafs.length; i++) {
			...
			if (length == lengths[i]) {
				...
				log.log(Level.FINE, "Truncating '" + names[i] + "' from " + lengths + " to " + lengths[i] + "bytes");
	...
	/**
	 * Returns a byte array containing the requested piece or null if the storage doesn't contain the piece yet.
	 */
	...
	private int getUncheckedPiece(int piece, byte[] bs, int off) throws IOException
	{
