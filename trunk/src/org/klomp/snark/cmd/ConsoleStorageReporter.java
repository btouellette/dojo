...
/**
 * Reports the status of allocation of space via logging data to the console.
 * 
...
public class ConsoleStorageReporter implements StorageListener
{
	...
	{
		log.log(Level.FINE, "Creating file '" + name + "' of length " + length + ": ");
	}
	...
	public void storageChecked(Storage storage, int num, boolean checked)
	{
		if (!allChecked && !checking) {
			...
			if (meta != null) {
				log.log(Level.INFO, "Checking existing " + meta.getPieces() + " pieces: ");
			}
			checking = true;
		}
		if (checking) {
			...
			log.log(Level.FINE, "Got " + (checked ? "" : "BAD ") + "piece: " + num);
		}
	}
